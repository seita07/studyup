class UsersController < ApplicationController
  def show
    ...
    gon.day = day
    gon.study_time = eachsubject_weekly_totaltime
    @day_totaltime = dayly_total_studytime
    @weekly_totaltime = weekly_total_studytime(@user) if @user.studytime
    @month_totaltime = month_total_studytime
    @totaltime = total_studytime
  end

  def guest_sign_in
    user = User.find_or_create_by!(name: 'guest', email: 'guest@example.com') do |users|
  ...
  def eachsubject_weekly_totaltime
    subject_total_study_list = []
    subject_list = ['国語', '現代文', '古文', '漢文', '数学', '数学I・A', '数学II・B', '数学III', '英語', '英単語', '英文法', '英文解釈', '英長文', 'リスニング', '日本史', '世界史', '倫理', '政治経済', '地理', '現代社会', '物理', '化学', '生物', '地学', '過去問']
